...
export default function CustomerMonitorPage() {
  ...
  useEffect(() => {
    ...
    // Listen for updates from the cashier screen
    channel.onmessage = (event) => {
      const data = event.data;
      if (data && data.type === "CART_UPDATE") {
        setCart(data.cart || []);
        setTotalNormal(data.totalNormal || 0);
        setDiscountAmount(data.discountAmount || 0);
        setGrandTotal(data.grandTotal || 0);
        setSelectedMember(data.selectedMember || null);
        setLastTransaction(data.lastTransaction || null);
      }
    };
  ...
  // Auto-hide thank you screen if cashier clears transaction or starts a new one
  useEffect(() => {
