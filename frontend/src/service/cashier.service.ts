...
export async function getActiveShiftService() {
  try {
...
export async function openShiftService(startCash: number) {
  const userId = await getCurrentProfileId();
  const { data, error } = await supabase
    .from("shifts")
    .insert({
...
export async function fetchExpensesService() {
  ...
  if (error) throw error;
  return { success: true, data: (data as ExpenseDTO[]) || [] };
}

export async function createExpenseService(payload: { name: string; amount: number; category: string }) {
  const userId = await getCurrentProfileId();
...
export async function voidTransactionService(id: number) {
...
export async function refundTransactionService(id: number, payload: { reason: string; items: Array<{ product_id: number; qty_refunded: number }> }) {
  const userId = await getCurrentProfileId();
...
// Backwards-compatible cashierDAO object
export const cashierDAO = {
  getMembers: fetchMembersService,
