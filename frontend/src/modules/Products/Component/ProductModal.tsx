...
import type { ProductDTO as Product } from "../DTO/products.dto";
...
export const ProductModal = ({
  ...
  useEffect(() => {
    ...
    const loadSuppliers = async () => {
      try {
        const { supplierDAO } = await import("@/modules/Suppliers/DAO/suppliers.dao");
        const res = await supplierDAO.getSuppliers();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingProduct?.id) {
        res = await productsDAO.update(existingProduct.id, payload);
      } else {
        res = await productsDAO.create(payload);
      }
