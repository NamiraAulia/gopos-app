...
const safeParseInt = (v: any, _field?: any, _label?: any) => {
  const n = parseInt(v, 10);
  return isNaN(n) ? 0 : n;
};
const safeParseFloat = (v: any, _field?: any, _label?: any) => {
  const n = parseFloat(v);
  return isNaN(n) ? 0 : n;
};
import { productsDAO } from "@/service/products.service";
...
const useBatchImport = () => {
  ...
  return {
    progress,
    startBatchImport: async (items: any[]) => {},
    cancelBatchImport: () => {},
    resetBatchImport: () => setProgress({ status: "idle", isRunning: false, currentChunk: 0, totalChunks: 0, successCount: 0, failCount: 0, failedCount: 0, isDone: false, errors: [], details: [] }),
    CHUNK_SIZE: 50
  };
};
  ...
  useEffect(() => {
    if (isOpen) {
      productsDAO.getBarcodes().then((res: any) => {
  ...
  const handleSubmitBatch = () => {
    const importableItems: any[] = rows
