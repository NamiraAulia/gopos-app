...
import type { MemberDTO, CreateMemberPayloadDTO } from "../DTO/member.dto";
...
export const memberDAO = {
  getMembers: async (): Promise<ApiResponse<MemberDTO[]>> => {
    try {
      ...
      const membersWithDebt: MemberDTO[] = (membersData || []).map((m: any) => ({
        ...m,
  ...
  createMember: async (payload: CreateMemberPayloadDTO): Promise<ApiResponse<MemberDTO>> => {
    try {
  ...
  editMember: async (id: number, payload: CreateMemberPayloadDTO): Promise<ApiResponse<MemberDTO>> => {
    try {
      const { data, error } = await supabase
        .from("members")
        .update({
  ...
  exportMembersCsv: (members: MemberDTO[]) => {
